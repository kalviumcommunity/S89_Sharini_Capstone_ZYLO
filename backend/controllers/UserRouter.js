...
// ✅ Get user by email (⚡ added this for profile updates)
router.get('/getuserbyemail/:email', async (req, res) => {
  try {
    ...
    console.error('Error in GET /getuserbyemail/:email:', error);
    res.status(500).json({ message: 'Internal Server Error' });
  }
});
...
router.put('/updateuserdetails/:id', async (req, res) => {
  try {
    ...
    console.error('Error in PUT /updateuserdetails/:email:', error);
    res.status(500).json({ message: 'Internal Server Error' });
  }
});
